...
# Carga dinámica de instituciones
...
class Product(Base):
    ...
    __table_args__ = (UniqueConstraint("institution_id", "url", name="uq_product_url"),)
...
def download_pdf(url: str, dest_dir: pathlib.Path) -> Optional[pathlib.Path]:
    try:
        ...
        dest = dest_dir / sanitize_filename(url)
        if dest.exists():
            return dest
...
def crawl_institution(slug: str, cfg: Dict[str, str | List[str]]):
    ...
    with SessionLocal() as db:
        ...
        for link in product_links:
            ...
            product = db.query(Product).filter_by(institution_id=inst.id, url=link).first()
            if not product:
            ...
            for pdf_url in pdf_links:
                if db.query(Document).filter_by(product_id=product.id, url=pdf_url).first():
                    continue
...
if FastAPI:
    ...
    def search(q: str, limit: int = 20, db: Session = Depends(get_session)):
        try:
            ...
            pattern = f"%{q}%"
            return db.scalars(select(Document).where(Document.text.like(pattern)).limit(limit)).all()
else:
    app = None
